...
export function WriterSidebar() {
  ...
  const [userName, setUserName] = useState("Expert Writer");
  const [userInitials, setUserInitials] = useState("EX");
  ...
  useEffect(() => {
    ...
    if (userStr) {
      try {
        ...
        setUserName(`${first} ${last}`.trim() || "Expert Writer");
        
        const fChar = first.charAt(0) || "W";
        const lChar = last.charAt(0) || "P";
        setUserInitials((fChar + lChar).toUpperCase());
      } catch (e) {
        console.error("Failed to parse user details in sidebar:", e);
      }
    }
  }, []);
  ...
  return (
    <aside className="fixed left-0 top-0 z-40 h-screen w-[280px] glass border-r border-white/20 flex flex-col hidden lg:flex overflow-hidden">
      ...
      <div className="p-4.5 border-t border-white/10 glass bg-white/5 dark:bg-slate-900/10 space-y-2 relative z-10">
        <Link href="/writer/profile" className="flex items-center gap-3 p-2.5 rounded-2xl hover:bg-white/5 transition-all group">
